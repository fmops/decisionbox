defmodule ExcisionWeb.ClassifierControllerTest do
  ...
  describe "create classifier" do
    ...
    test "renders classifier when data is valid", %{conn: conn, decision_site: decision_site} do
      conn = post(conn, ~p"/api/decision_sites/#{decision_site}/classifiers", 
    ...
    test "renders errors when data is invalid", %{conn: conn, decision_site: decision_site} do
      conn = post(conn, ~p"/api/decision_sites/#{decision_site}/classifiers", classifier: @invalid_attrs)
  ...
  describe "update classifier" do
    ...
    test "renders classifier when data is valid", %{conn: conn, classifier: %Classifier{id: id} = classifier} do
      conn = put(conn, ~p"/api/decision_sites/#{classifier.decision_site_id}/classifiers/#{classifier}", classifier: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, classifier: classifier} do
      conn = put(conn, ~p"/api/decision_sites/#{classifier.decision_site_id}/classifiers/#{classifier}", classifier: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete classifier" do
    ...
    test "deletes chosen classifier", %{conn: conn, classifier: classifier} do
      conn = delete(conn, ~p"/api/decision_sites/#{classifier.decision_site_id}/classifiers/#{classifier}")
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, ~p"/api/decision_sites/#{classifier.decision_site_id}/classifiers/#{classifier}")
      end
    end
  end
